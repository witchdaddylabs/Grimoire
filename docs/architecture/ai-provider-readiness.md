...
Grimoire remains local-first. Ollama is the default local provider. Cloud providers are optional BYOK providers and require explicit user configuration, explicit provider selection, the user's API key, and an in-product disclosure before Vault context or Canvas text is sent off-device.

## Current Co-Writer Path

1. Search local SQLite FTS chunks.
2. Build grounded context from retrieved Vault chunks and active Canvas text.
...
- Provider keys are stored in the OS credential store (macOS Keychain / Windows Credential Manager) through the Rust secret layer (the `keyring` crate).
